...
static CORE_FREQUENCY: OnceCell<(u64, u64)> = OnceCell::new();
#[cfg(all(target_arch = "aarch64", target_os = "macos"))]
static EFFICIENCY_CLUSTER_TYPE: OnceCell<std::collections::HashMap<u32, CpuClusterType>> =
  OnceCell::new();
      ...
      unsafe {
        let (efficiency_core_frequency, performance_core_frequency) = CORE_FREQUENCY
          .get_or_try_init(|| {
            ...
            let platform_device_dictionary =
              IOServiceMatching(b"AppleARMIODevice\0".as_ptr().cast());
        ...
        let efficiency_cluster_type = EFFICIENCY_CLUSTER_TYPE.get_or_try_init(|| {
